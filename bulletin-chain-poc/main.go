...
func init() {
	...
	flag.StringVar(&ethRpcUrl, "eth-rpc-url", "", "Ethereum RPC URL")
	flag.StringVar(&stakingAddress, "staking-address", "", "Staking contract address")
	flag.Uint64Var(&blocksPerEpoch, "blocks-per-epoch", 100, "Number of DAG blocks per epoch")
	flag.Parse()
...
func run(ctx context.Context) error {
	...
	stakingConfig := &dag.StakingConfig{
		EthRpcUrl:      ethRpcUrl,
		StakingAddress: stakingAddress,
		BlocksPerEpoch: blocksPerEpoch,
	}

	dagger := createDAGChain(broadcaster, mcastPool, stakingConfig, privKey)
	dagger.Start()
	defer dagger.Stop()
...
func createDAGChain(broadcaster *gossip.Broadcaster, mcastPool *bapl.MulticastPool, stakingConfig *dag.StakingConfig, privKey crypto.PrivKey) *dag.Chain {
	return dag.NewChainWithStakingConfig(broadcaster, mcastPool, stakingConfig, privKey.PubKey())
}
